...
class ModelWrapper(LightningModule):
    # logger: Optional[logging.RootLogger]
    encoder: nn.Module
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Run the model.
        gs_list = self.encoder(
            batch["context"], self.global_step, False, scene_names=batch["scene"]
        )
        offset = []
        for i in range(len(gs_list)):
            offset.append(gs_list[i][0].shape[0])
        ...
        # self.vlog.info("train/psnr_probabilistic", psnr_probabilistic.mean())
        # self.vlog.info('train/psnr_probabilistic : {}'.format(psnr_probabilistic.mean()))
        
        # Compute and log loss.
        total_loss = 0
        for loss_fn in self.losses:
            loss = loss_fn.forward(output, batch, gs_list, self.global_step)
        ...
        if (
            ...
            # )
            self.vlog.info('train step {}, loss = {}, psnr_probalisitic = {}, offset = {}'.format(self.global_step, total_loss, psnr_probabilistic.mean(), offset))
    ...
    def test_step(self, batch, batch_idx):
        ...
        # flops_counter = TorchFLOPsByFX(self.encoder)
        ...
        #     )
        #     self.vlog.info("psnr = {}, ssim = {}, lpips = {}.".format(sum(self.test_step_outputs[f"psnr"]) / len(self.test_step_outputs[f"psnr"]),
        #                                                               sum(self.test_step_outputs[f"ssim"]) / len(self.test_step_outputs[f"ssim"]),
        #                                                               sum(self.test_step_outputs[f"lpips"]) / len(self.test_step_outputs[f"lpips"])))
        # # print(self.test_step_outputs[f"psnr"])
        # # assert 1==0
        ...
        psnr = compute_psnr(rgb_gt, images_prob).mean().item()
        import json, os, time
